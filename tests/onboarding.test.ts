...
const EXPECTED_STARK_KEY_PAIR_MAINNET = {
  publicKey:
    '0x023fdc1b2adf55afcfa7b6c9af35d27d4128a084a613157b762e2f87abb83a80',
  publicKeyYCoordinate:
    '062de810a15a5f50790d9c87eb83019a2d68ab19e98fafbd262d600df1a16660',
  privateKey:
    '0x077b73fed7bf763d3c99f9724b9df2acb86f0b529675fe89b74c9d670ac64d8d',
};
...
const EXPECTED_STARK_KEY_PAIR_ROPSTEN = {
  publicKey:
    '0x0464f315d6fc4a3a1e345f1d3836d3538f96a1377a47e61272c4ab63c9d545ec',
  publicKeyYCoordinate:
    '06fa328262fc7542ce776a70235e76280ab305fa6e632cbc195819065f7f97f9',
  privateKey:
    '0x031c0a807c594b0821b0191ab37332adbd5a097189b472812f89bb50d7bf15ee',
};
...
describe('Onboarding module', () => {
  describe('mainnet, with a web3 provider', () => {
    ...
    it('derives the default STARK key pair', async () => {
      const keyPair = await onboardingMainnetRemote.deriveStarkKey(
        GANACHE_ADDRESS
      );
    ...
    it('derives the default API key pair', async () => {
      const apiKey = await onboardingMainnetRemote.recoverDefaultApiCredentials(
        GANACHE_ADDRESS
      );
  ...
  describe('Ropsten, with a web3 provider', () => {
    ...
    it('derives the default STARK key pair', async () => {
      const keyPair = await onboardingRopstenRemote.deriveStarkKey(
        GANACHE_ADDRESS
      );
    ...
    it('derives the default API key pair', async () => {
      const apiKey = await onboardingRopstenRemote.recoverDefaultApiCredentials(
        GANACHE_ADDRESS
      );
  ...
  describe('mainnet, with a local Ethereum private key', () => {
    ...
    it('derives the default STARK key pair', async () => {
      const keyPair = await onboardingMainnetLocal.deriveStarkKey(
        GANACHE_ADDRESS
      );
      expect(keyPair).toStrictEqual(EXPECTED_STARK_KEY_PAIR_MAINNET);
    });
    ...
    it('derives the default API key pair', async () => {
      const apiKey = await onboardingMainnetLocal.recoverDefaultApiCredentials(
        GANACHE_ADDRESS
      );
      expect(apiKey).toStrictEqual(EXPECTED_API_KEY_CREDENTIALS_MAINNET);
    });
  ...
  describe('Ropsten, with a local Ethereum private key', () => {
    ...
    it('derives the default STARK key pair', async () => {
      const keyPair = await onboardingRopstenLocal.deriveStarkKey(
        GANACHE_ADDRESS
      );
      expect(keyPair).toStrictEqual(EXPECTED_STARK_KEY_PAIR_ROPSTEN);
    });
    ...
    it('derives the default API key pair', async () => {
      const apiKey = await onboardingRopstenLocal.recoverDefaultApiCredentials(
        GANACHE_ADDRESS
      );
      expect(apiKey).toStrictEqual(EXPECTED_API_KEY_CREDENTIALS_ROPSTEN);
    });
